...
public class JiebaSegmenter2 {
	  private static WordDictionary2 wordDict = WordDictionary2.getInstance();
	    ...
	    private Map<Integer, List<Integer>> createDAG(String sentence) {
	        ...
	        DictSegment2 trie = wordDict.getTrie();
	        char[] chars = sentence.toCharArray();
	        int N = chars.length;
	        int i = 0, j = 0;
	        while (i < N) {
	            Hit2 hit = trie.match(chars, i, j - i + 1);
	    ...
	    public List<SegToken> process(String paragraph, SegMode mode) {
	        ...
	        String[] paragraph_exp = paragraph.split("[ ]");
	        int offset = 0;
	        for (int i = 0; i < paragraph_exp.length; ++i) {
	        	
	           // char ch = CharacterUtil.regularize(paragraph_exp[i].toString());
	           
	                if (paragraph_exp[i].toString().length() > 0) {
	                    // process
	                    if (mode == SegMode.SEARCH) {
	                        for (String word : sentenceProcess(paragraph_exp[i].toString().toString())) {
	                            tokens.add(new SegToken(word, offset, offset += word.length()));
	                        }
	                    }
	                    else {
	                        for (String token : sentenceProcess(paragraph_exp[i].toString().toString())) {
	                            if (token.length() > 2) {
	                                String gram2;
	                                int j = 0;
	              ...
	              //  System.out.println(paragraph_exp[i].toString());
	                if (wordDict.containsWord(paragraph_exp[i].toString()))
	                    tokens.add(new SegToken(paragraph_exp[i].toString(), offset, ++offset));
	                else
	                    tokens.add(new SegToken(paragraph_exp[i].toString(), offset, ++offset));
